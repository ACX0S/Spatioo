...
-- Create function to populate vagas for existing estacionamentos
...
BEGIN
  -- Loop through all estacionamentos that don't have vagas yet
  FOR estacionamento_record IN 
    SELECT e.id, e.numero_vagas
    FROM public.estacionamento e
    LEFT JOIN public.vagas v ON e.id = v.estacionamento_id
    WHERE v.id IS NULL
    GROUP BY e.id, e.numero_vagas
  LOOP
    -- Create vagas for this estacionamento
    INSERT INTO public.vagas (estacionamento_id, numero_vaga, tipo_vaga, status)
    SELECT 
      estacionamento_record.id,
      'Vaga ' || generate_series(1, estacionamento_record.numero_vagas),
      'comum',
      'disponivel'
    FROM generate_series(1, estacionamento_record.numero_vagas);
    
    total_created := total_created + estacionamento_record.numero_vagas;
  END LOOP;
